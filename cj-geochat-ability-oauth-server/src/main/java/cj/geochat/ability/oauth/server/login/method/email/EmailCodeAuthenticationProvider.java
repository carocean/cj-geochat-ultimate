...
public class EmailCodeAuthenticationProvider implements AuthenticationProvider {
    ...
    public EmailCodeAuthenticationProvider(IVerifyCodeService verifyCodeService, UserDetailsService userDetailsService) {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        EmailCodeAuthenticationToken adminLoginToken = (EmailCodeAuthenticationToken) authentication;
//        System.out.println("===进入Admin密码登录验证环节====="+ JSON.toJSONString(adminLoginToken));
        ...
        if (authentication.getCredentials().toString().equals(verifyCodeInfo.getCode())) {
            return new EmailCodeAuthenticationToken(userDetails, verifyCodeInfo.getCode(), userDetails.getAuthorities());
        }
